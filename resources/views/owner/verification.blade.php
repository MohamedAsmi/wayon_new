				...
				<div class="container-fluid min-height">
					<div class="col-md-12">
						...
						<div class="row justify-content-center mt-5">
							<div class="col-md-12 p-0">
								...
								<div class="main-panel border main-panelbg">
									...
									<ul class="list-layout edit-verifications-list">
										...
										@if((Auth::guard('owner')->user()->email == 'no') && (Auth::guard('owner')->user()->ownerverification->facebook == 'no') && (Auth::guard('owner')->user()->ownerverification->google == 'no'))
											<div class="alert alert-success mt-3" role="alert">
											No Verification Available
											</div>
										@else
											@if(Auth::guard('owner')->user()->email == 'yes')
											<li class="edit-verifications-list-item">
												<h4 class="font-weight-700 text-16">{{ trans('messages.users_dashboard.email_address') }}</h4>
												<p class="pl-4">{{ trans('messages.profile.you_have_confirmed_email') }} <b>{{ Auth::guard('owner')->user()->email }}</b>.  {{ trans('messages.profile.email_verified') }}
												</p>
											</li>
											@endif
											<hr>
										
											@if(Auth::guard('owner')->user()->ownerverification->facebook == 'yes')
											<li class="edit-verifications-list-item">
											...
											@if(Auth::guard('owner')->user()->ownerverification->google == 'yes')
											<li class="edit-verifications-list-item">
							...
							<div class="card card-default">
								@if(!(Auth::guard('owner')->user()->email == 'yes' && Auth::guard('owner')->user()->ownerverification->facebook == 'yes' && Auth::guard('owner')->user()->google == 'yes'))
								...
								<div class="p-4">
									<ul>
									@if(Auth::guard('owner')->user()->email == 'no')
										<li>
										...
										<div class="row pl-4 pt-1">
											<div class="col-md-9">
											<p>
												{{ trans('messages.profile.email_verification') }} <b>{{ Auth::guard('owner')->user()->email }}</b>.
											</p>
											</div>
									...
									@if(Auth::guard('owner')->user()->ownerverification->facebook == 'no')
										<li>
									...
									@if(Auth::guard('owner')->user()->ownerverification->google == 'no')
										<li>
											<h4 class="font-weight-700 text-16 mt-4">
