	...
	<div class="row mb-4">
		<div class="col-md-8 card p-5">
			...
			<div class="row flex-column-reverse flex-md-row mt-4 m-0">
				<div class="col-md-9 p-0">
					...
					@if($result->host_id == Auth::guard('owner')->user()->id)
				@if($result->status == 'Pending')
				<div>
			...
			@endif
				...
				<div class="col-md-3">
					<div class="media-photo-badge text-center">
						<a href="{{ secure_url('/') }}/users/show/{{ $result->users->id }}" ><img alt="{{ $result->users->first_name }}" class="" src="{{ $result->users->profile_src }}" title="{{ $result->users->first_name }}"></a>
					</div>
					<p class="font-weight-700 mb-0 text-center"><a href="{{ secure_url('/') }}/users/show/{{ $result->users->id }}" class="verification_user_name">{{ $result->users->first_name }}</a></p>
        ...
        <div class="col-md-4">
            <div class="card mt-3 mb-5 p-3">
                <a href="{{ secure_url('/') }}/properties/{{ $result->properties->slug}}">
	...
	<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				...
				<div class="modal-body p-4">
					<form accept-charset="UTF-8" action="{{ url('owner/booking/accept/'.$booking_id) }}" id="accept_reservation_form" method="post" name="accept_reservation_form">
						{{ csrf_field() }}
						<div class="row">
							<div class="col-md-12">
							...
							<div class="col-md-12">
								<div class="row mt-4">
									...
									<div class="col-sm-11 p-0 text-16 text-justify">
										<label class="label-inline" for="tos_confirm">{{ trans('messages.booking_detail.check_box_agree') }} <br><a href="{{ secure_url('/') }}/host_guarantee" target="_blank" class="font-weight-700">{{ trans('messages.booking_detail.guarantee_term_condition') }}</a> <br><a href="{{ secure_url('/') }}/guest_refund" target="_blank" class="font-weight-700">{{ trans('messages.booking_detail.refund_policy_term') }}</a>, {{ trans('messages.booking_detail.and') }} and <a href="{{ secure_url('/') }}/terms_of_service" target="_blank" class="font-weight-700">{{ trans('messages.booking_detail.term_of_service') }}</a>.</label>
									</div>
								</div>
							</div>
