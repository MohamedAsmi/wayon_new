...
class BookingsDataTable extends DataTable
{
    ...
    public function query()
    {
        ...
        $bookings = Bookings::join('properties', function ($join) {
            ...
            ->Join('owners as o', function ($join) {
                $join->on('o.id', '=', 'bookings.host_id');
            })
            ->select(['bookings.id as id', 'o.username as host_name', 'users.first_name as guest_name', 'bookings.property_id as property_id', 'properties.name as property_name','bookings.total AS total_amount','bookings.payment_method_id', 'bookings.status', 'bookings.created_at as created_at', 'bookings.updated_at as updated_at', 'bookings.start_date', 'bookings.end_date', 'bookings.guest', 'bookings.host_id', 'bookings.user_id', 'bookings.total', 'bookings.currency_code', 'bookings.service_charge', 'bookings.host_fee', 'bookings.iva_tax', 'bookings.accomodation_tax']);
        if (isset($user_id)) {
