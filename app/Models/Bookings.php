...
class Bookings extends Model
{
    ...
    {
        return $this->belongsTo('App\Models\Owner', 'host_id', 'id');
    }
    ...
    public function getBookingOwnerLists()
    {
        $bookings = Bookings::join('properties', function ($join) {
                        ...
                        ->leftJoin('owners as o', function ($join) {
                                $join->on('o.id', '=', 'bookings.host_id');
                        })
                        ->where('bookings.host_id','=',Auth::guard('owner')->user()->id)
                        ->select(['bookings.id as id', 'o.username as host_name', 'users.first_name as guest_name', 'bookings.property_id as property_id', 'properties.name as property_name', \DB::raw('CONCAT(bookings.total) AS total_amount'), 'bookings.status', 'bookings.created_at as created_at', 'bookings.updated_at as updated_at', 'bookings.start_date', 'bookings.end_date', 'bookings.guest', 'bookings.host_id', 'bookings.user_id', 'bookings.total', 'bookings.currency_code', 'currency.symbol', 'bookings.service_charge', 'bookings.host_fee'])
                        ->take(5)
                        ->get();
        return $bookings;
    }
    ...
    public function getUserBookingLists()
    {
        $bookings = Bookings::join('properties', function ($join) {
                                $join->on('properties.id', '=', 'bookings.property_id');
        })
