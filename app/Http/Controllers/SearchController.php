...
class SearchController extends Controller
{
    ...
    function searchResult(Request $request)
    {
        ...
        $users_where['owners.status'] = 'Active';
        ...
        $properties = Properties::with([
           ...
           ->whereHas('property_address', function ($query) use ($minLat, $maxLat, $minLong, $maxLong) {
                $query->whereRaw("latitude between ($minLat - 9) and ($maxLat + 9) and longitude between ($minLong - 9 ) and ($maxLong + 9)");
            })
            ->whereHas('property_price', function ($query) use ($min_price, $max_price, $currency_rate) {
                ...
                // $query->whereRaw('((price / currency.rate) * ' . $currency_rate . ') >= ' . 1 . ' and ((price / currency.rate) * ' . $currency_rate . ') <= ' . 10000);
            })
            // ->whereHas('owners', function ($query) use ($users_where) {
