...
use App\DataTables\OwnerPayoutListDataTable;
...
class PayoutController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['payouts'] = PayoutSetting::with('payment_methods')->where(['user_id' => Auth::guard('owner')->user()->id])->paginate(Session::get('row_per_page'));
        ...
        $data['walletBalance'] = Wallet::where(['user_id' => Auth::guard('owner')->user()->id])->first();
        return view('owner.payouts.view', $data);
    }
      ...
      if (!$request->isMethod('post')) {
          ...
          if ($request->payout_type == 1) {
              ...
              if ($validator->fails()) {
                  ...
                  $payoutSetting->user_id               = Auth::guard('owner')->user()->id;
                  ...
                  $paymentAccount->user_id               = Auth::guard('owner')->user()->id;
    ...
    public function payoutList(OwnerPayoutListDataTable $dataTable)
    {
        $data['title']= 'Payouts';
        ...
        $data['payouts'] = PayoutSetting::where(['user_id' => Auth::guard('owner')->user()->id])->get();

        $data['currentCurrency'] = Currency::getAll()->firstWhere('code',
        Session::get('currency') ?? $this->helper->getCurrentCurrencyCode());
        // dd($data);

        return $dataTable->render('owner.payoutlists.view',$data);
    }
