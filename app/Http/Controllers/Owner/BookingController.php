...
class BookingController extends Controller
{
    ...
    */
    public function index(Request $request)
    {
        ...
        if (! $data['result'] || $data['result']->host_id != Auth::guard('owner')->user()->id ) {
    ...
    */
    public function accept(Request $request, EmailController $email)
    {
       ...
       twilioSendSms($booking->users->formatted_phone, $requestBookingConfirm);
        ...
        return redirect('owner/my-bookings');
    }
    ...
    */
    ...
    public function myBookings(Request $request)
    {
        ...
        $data['title']   = "Bookings";
        $query = Bookings::with(['host','properties'])
        ->where('host_id', Auth::guard('owner')->user()->id)
        ->where($params)->orderBy('id', 'desc');
        $data['bookings'] = $query->paginate(Session::get('row_per_page'));
        // dd($query->toSql());

        return view('owner.booking.my_bookings', $data);
    }
