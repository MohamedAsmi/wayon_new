...
use App\Http\Helpers\PaymentService;
use App\Models\{
    Bank,
    BankDate,
    Payouts,
...
class PaymentController extends Controller
{
    ...
    public function createBooking(Request $request)
    {
        ...
        if ($request->payment_method == 'stripe') {
           ...
           $data = [
            ...
            'phone'=> Auth::user()-> formatted_phone
        ];
        // $code = $this->store($data);
        return redirect()->route('makePayment')->with('requesdata', $data);
    ...
    public function makePhonePePayment(Request $request)
    {
        ...
        $requesdata = session('requesdata');
        $price_list =(object) $requesdata['price_list'];
    //   dd($price_list);
        $data = array(
            'merchantId' => 'M1XPPGDWPPT8',
            'merchantTransactionId' => 'MT'.str_random(16),
            'merchantUserId' => 'MUID123',
        ...
        $string = $encode . '/pg/v1/pay' . 'bd9ae94b-56ea-40f0-9efa-15f639fd1a26';
        $sha256 = hash('sha256', $string);
        $finalXHeader = $sha256 . '###' . '2';
    ...
    public function paymentsuccess(Request $request){
        ...
        if($request->code == "PAYMENT_SUCCESS" ){
            ...
            // $payment->save();
            $code = $this->store($request);
            $price_list = (object)$code->price_list;


            $this->helper->one_time_message('success', trans('messages.booking_request.request_has_sent'));
            return redirect('booking/requested?code='.$price_list->currency);
        } 
        else
        {
            $this->helper->one_time_message('Error', trans('Payment Failed ...!'));
    ...
    public function store($data)
    {
        ...
        $booking->bank_id             = $data->bank_id ?? null;
        $booking->total_night       = $price_list->total_nights ?? null;
        $booking->per_night         = $this->helper->convert_currency('', $currencyDefault->code, $price_list->property_price);

        $booking->custom_price_dates= isset($price_list->different_price_dates_default_curr) ? json_encode($price_list->different_price_dates_default_curr) : null ;

        $booking->base_price        = $this->helper->convert_currency('', $currencyDefault->code, $price_list->subtotal);
        $booking->cleaning_charge   = $this->helper->convert_currency('', $currencyDefault->code, $price_list->cleaning_fee);
        $booking->guest_charge      = $this->helper->convert_currency('', $currencyDefault->code, $price_list->additional_guest);
        $booking->iva_tax           = $this->helper->convert_currency('', $currencyDefault->code, $price_list->iva_tax);
        $booking->accomodation_tax  = $this->helper->convert_currency('', $currencyDefault->code, $price_list->accomodation_tax);
        $booking->security_money    = $this->helper->convert_currency('', $currencyDefault->code, $price_list->security_fee);
        $booking->service_charge    = $service_fee  = $this->helper->convert_currency('', $currencyDefault->code, $price_list->service_fee);
        $booking->host_fee          = $host_fee     = $this->helper->convert_currency('', $currencyDefault->code, $price_list->host_fee);
        $booking->total             = $total_amount = $this->helper->convert_currency('', $currencyDefault->code, $price_list->total);

        $booking->currency_code     = $currencyDefault->code;
        $booking->transaction_id    = $data->transaction_id ?? " ";
        $booking->payment_method_id = $data->payment_method_id ?? 5;
        ...
        $booking->booking_type      = Session::get('payment_booking_type') ?? 'Pending';

        foreach ($price_list->date_with_price as $key => $value) {
            $value =(object) $value;
